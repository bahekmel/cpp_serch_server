...
SearchServer::SearchServer(const std::string& stop_words_text)  // Invoke delegating constructor
    : SearchServer(SplitIntoWords(stop_words_text))             // from string container
{
}


SearchServer::SearchServer(const std::string_view stop_words_view)  // Invoke delegating constructor
    : SearchServer(SplitIntoWords(stop_words_view))           // from string container
{
}
...
void SearchServer::QueryPar::SortUniq(bool sort_plus)
{
    if (sort_plus) {
...
void SearchServer::QueryPar::SortUniq() {
    SortUniq(true);
    SortUniq(false);
}
...
SearchServer::QueryPar SearchServer::ParseQueryPar(std::string_view text) const {
    const std::vector<std::string_view> query_words = SplitIntoWords(text);
    SearchServer::QueryPar result;
