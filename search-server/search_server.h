...
private:
    ...
    struct QueryPar
    {
        ...
        explicit QueryPar(size_t size_plus, size_t size_minus) : plus_words(size_plus), minus_words(size_minus)
        {
        }

        explicit QueryPar(size_t size) : plus_words(size), minus_words(size)
        {
        }
    ...
    QueryPar ParseQueryPar(std::string_view) const;
